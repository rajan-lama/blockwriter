 ...
 */
import type { CustomValidator, NormalizedField, Operator, Validator } from './field-api';
export type SelectionOrUpdater = string[] | ((prev: string[]) => string[]);
export type SetSelection = (selection: SelectionOrUpdater) => void;
export type FieldType<Item> = Pick<NormalizedField<Item>, 'type' | 'render' | 'sort' | 'enableSorting' | 'enableGlobalSearch' | 'format' | 'getValueFormatted'> & {
    Edit: string | null;
