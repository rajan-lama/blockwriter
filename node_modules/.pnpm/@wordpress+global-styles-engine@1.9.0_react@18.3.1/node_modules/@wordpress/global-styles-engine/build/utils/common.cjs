...
var PRESET_METADATA = [
  ...
  },
  {
    ...
    valueFunc: (preset, settings) => (0, import_typography.getTypographyFontSizeValue)(preset, settings),
    valueKey: "size",
...
function findInPresetsBy(settings, blockName, presetPath = [], presetProperty = "slug", presetValueValue) {
  const orderedPresetsByOrigin = [
    blockName ? (0, import_object.getValueFromObjectPath)(settings, [
      "blocks",
      blockName,
      ...presetPath
    ]) : void 0,
    (0, import_object.getValueFromObjectPath)(settings, presetPath)
  ].filter(Boolean);
...
function getValueFromCustomVariable(features, blockName, variable, path = []) {
  const result = (blockName ? (0, import_object.getValueFromObjectPath)(features?.settings ?? {}, [
    "blocks",
    blockName,
    "custom",
    ...path
  ]) : void 0) ?? (0, import_object.getValueFromObjectPath)(features?.settings ?? {}, [
    "custom",
    ...path
  ]);
  if (!result) {
...
function getValueFromVariable(features, blockName, variable) {
  if (!variable || typeof variable !== "string") {
    if (typeof variable === "object" && variable !== null && "ref" in variable && typeof variable.ref === "string") {
      const resolvedVariable = (0, import_object.getValueFromObjectPath)(
        features,
        variable.ref
      );
