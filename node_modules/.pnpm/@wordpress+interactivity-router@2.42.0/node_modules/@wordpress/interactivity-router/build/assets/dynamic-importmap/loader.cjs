...
__export(loader_exports, {
  importPreloadedModule: () => importPreloadedModule,
  initPromise: () => initPromise,
  initialImportMap: () => initialImportMap,
...
function resolveDeps(load, seen) {
  ...
  const [imports, exports2] = load.analysis;
...
//# sourceURL=${depLoad.responseUrl}?cycle`
    ...
    if (load.shellUrl) {
      resolvedSource += `
;import{u$_}from'${load.shellUrl}';try{u$_({${exports2.filter((e) => e.ln).map(({ s, e, ln }) => `${source.slice(s, e)}:${ln}`).join(",")}})}catch(_){};
`;
    }
...
function getOrCreateLoad(url, fetchOpts, parent) {
  ...
  load.fetchPromise = (async () => {
    let source;
    ({ responseUrl: load.responseUrl, source } = await (fetchCache[url] || (0, import_fetch.fetchModule)(url, fetchOpts, parent)));
    try {
  ...
  load.linkPromise = load.fetchPromise.then(async () => {
    ...
    load.deps = (await Promise.all(
      load.analysis[0].map(async ({ n, d }) => {
        ...
        const responseUrl = (0, import_resolver.resolve)(
          n,
...
async function importPreloadedModule(load) {
  const module2 = await dynamicImport(load.blobUrl);
  if (load.shellUrl) {
    (await dynamicImport(load.shellUrl)).u$_(module2);
  }
  return module2;
}
