"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// packages/block-library/src/query-title/use-post-type-label.js
var use_post_type_label_exports = {};
__export(use_post_type_label_exports, {
  usePostTypeLabel: () => usePostTypeLabel
});
module.exports = __toCommonJS(use_post_type_label_exports);
var import_core_data = require("@wordpress/core-data");
var import_data = require("@wordpress/data");
function usePostTypeLabel(contextPostType) {
  const currentPostType = (0, import_data.useSelect)((select) => {
    const { getCurrentPostType } = select("core/editor");
    return getCurrentPostType();
  }, []);
  return (0, import_data.useSelect)(
    (select) => {
      const { getPostType } = select(import_core_data.store);
      const postTypeSlug = contextPostType || currentPostType;
      const postType = getPostType(postTypeSlug);
      return {
        postTypeLabel: postType ? postType.labels.singular_name : ""
      };
    },
    [contextPostType, currentPostType]
  );
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  usePostTypeLabel
});
//# sourceMappingURL=use-post-type-label.cjs.map
