(function (wpI18n, wpBlocks, wpElement, wpBlockEditor, wpComponents) {
  ...
  const { InspectorControls, RichText, PanelColorSettings, InnerBlocks } =
    wpBlockEditor;
  const { RangeControl, PanelBody, BaseControl, SelectControl, ToggleControl } =
    wpComponents;
  ...
  class AdvAccordion extends Component {
    ...
    componentWillMount() {
      ...
      if (attributes.changed !== true) {
        if (
          typeof currentBlockConfig === 'object' &&
      ...
      return (
        <Fragment>
          <InspectorControls>
            ...
            />
            <PanelBody
              title={__('Border Settings', 'prosys-power-block')}
              initialOpen={false}
            >
              <SelectControl
                ...
                options={[
                  {
                    label: __('Solid', 'prosys-power-block'),
                    value: 'solid',
                  },
                  {
                    label: __('Dashed', 'prosys-power-block'),
                    value: 'dashed',
                  },
                  {
                    label: __('Dotted', 'prosys-power-block'),
                    value: 'dotted',
                  },
                ]}
            ...
            <PanelBody
              title={__('Accordions State', 'prosys-power-block')}
              initialOpen={false}
            >
              <ToggleControl
                label={__('Initial Collapsed', 'prosys-power-block')}
                help={__(
                  'Make all accordions collapsed by default, enable this setting to apply to all accordions.',
                  'prosys-power-block',
                )}
  ...
  registerBlockType('prosys-power-block/accordion', {
    ...
    icon: {
      ...
      foreground:
        typeof advgbBlocks !== 'undefined' ? advgbBlocks.color : undefined,
    },
    category: 'ppb-category',
    keywords: [
      __('accordion', 'prosys-power-block'),
      __('list', 'prosys-power-block'),
      __('faq', 'prosys-power-block'),
    ],
