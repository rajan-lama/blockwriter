...
const PostCarouselSettingsPanel = ( { attributes, setAttributes } ) => {
	const {
		postType,
		perPage,
		columns,
	...
	return (
		<>
			<PanelBody
				title={ __( 'Carousel Query', 'blockwriter' ) }
				initialOpen={ true }
			>
				<SelectControl
				...
				<RangeControl
					label={ __( 'Number of slides', 'blockwriter' ) }
					value={ perPage }
					min={ 1 }
					max={ 24 }
					step={ 1 }
				...
				<RangeControl
					label={ __( 'Slides visible at once', 'blockwriter' ) }
					value={ columns }
					min={ 1 }
					max={ 4 }
					step={ 1 }
				...
				{ postType === 'post' && categories.length > 0 && (
					<>
						...
						<div
							className="bw-post-carousel-categories"
							style={ {
								maxHeight: 180,
								overflowY: 'auto',
								marginBottom: 12,
			...
			<PanelBody
				title={ __( 'Carousel Display', 'blockwriter' ) }
				initialOpen={ false }
			>
				<ToggleControl
				...
				<TextControl
					...
					help={ __(
						'Leave empty to hide the read more link.',
						'blockwriter',
					) }
					value={ readMoreText }
			...
			<PanelBody
				title={ __( 'Carousel Controls', 'blockwriter' ) }
				initialOpen={ false }
			>
				<ToggleControl
				...
				{ autoplay && (
					<RangeControl
						label={ __( 'Interval (ms)', 'blockwriter' ) }
						value={ interval }
						min={ 2000 }
						max={ 15000 }
						step={ 500 }
				...
				<ToggleControl
					label={ __( 'Show arrows', 'blockwriter' ) }
					checked={ !! showArrows }
					onChange={ ( value ) => setAttributes( { showArrows: value } ) }
				/>

				<ToggleControl
					label={ __( 'Show dots', 'blockwriter' ) }
					checked={ !! showDots }
...
export default PostCarouselSettingsPanel;
